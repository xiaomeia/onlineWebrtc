...
export const useLoginStore = defineStore('login', {
  ...
  getters: {
    ...
    getMobileSign: (state) => state.mobileSign,
    getSignMobile: (state) => state.signMobile,
    getLoginInfo: (state) => state.loginInfo,
    getLoginInfoList: (state) => state.loginInfoList
  },

  actions: {
    actionChangeAppStatus(status) {
      this.appStatus = status;
    },

    actionAddLoginLog(log) {
      let sr = [...this.loginLog];
      sr.push(log);
      if (sr.length > 3) {
        sr = sr.slice(-3);
      }
    ...
    actionSetMobileSign(sign) {
      this.mobileSign = sign;
    },

    actionSetSignMobile(mobile) {
      this.signMobile = mobile;
    },

    actionSetLoginInfo(info) {
      this.loginInfo = info;
    },

    actionSetLoginInfoList(list) {
