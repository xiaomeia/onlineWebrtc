...
export const useChatviewStore = defineStore('chatview', {
  ...
  getters: {
    ...
    getIntentMessage: (state) => state.intentMessage,
    getMessages: (state) => state.messages,
    getMessageTime: (state) => state.time,
    getRosterInfo: (state) => state.rosterMap[state.sid] || {},
    getGroupInfo: (state) => state.groupMap[state.sid] || {},
    getMemberList: (state) => state.memberlistMap[state.sid] || [],
    getAdminList: (state) => state.adminListMap[state.sid] || [],
  ...
  actions: {
    ...
    setRosterInfo(x) {
      this.rosterMap[this.sid] = x;
    },

    setGroupInfo(x) {
      this.groupMap[this.sid] = x;
    },

    setMemberList(x) {
      this.memberlistMap[this.sid] = x;
    },

    setAdminList(x) {
      this.adminListMap[this.sid] = x;
    },

    setQud(x) {
      this.queryHistoryMessageId = x;
    },

    recordHistoryQuery() {
      let times = this.queryHistoryRecords[this.sid] || 0;
      times++;
      this.queryHistoryRecords[this.sid] = times;
    },

    async actionOpenGroup() {
      const groupManage = useGroupManageStore();
      const userManage = useUserManageStore();

      groupManage.openGroup(this.sid);
      groupManage.readGroupMessage(this.sid);

      try {
        const adminList = await groupManage.asyncGetAdminList({ group_id: this.sid });
      ...
      try {
        const groupInfo = await groupManage.asyncGetGroupInfo(this.sid, true);
    ...
    async actionPreOpenGroup(x) {
      const groupManage = useGroupManageStore();
      const userManage = useUserManageStore();

      groupManage.openGroup(x.sid);
      groupManage.readGroupMessage(x.sid);

      try {
        const adminList = await groupManage.asyncGetAdminList({ group_id: x.sid });
        this.setAdminList(adminList);
      } catch (err) {
        console.error('Failed to GetAdminList, error:', err);
      }
      ...
      try {
        const groupInfo = await groupManage.asyncGetGroupInfo(x.sid, true);
        this.setGroupInfo(groupInfo);
      } catch (err) {
        console.error('Failed to GetGroupInfo, error:', err);
      }
    ...
    async actionGetGroupAdminList(x) {
      const groupManage = useGroupManageStore();
    ...
    actionSetType(x) {
      ...
      if (this.sid !== x.sid || this.viewType !== x.type) {
        this.setSid(x.sid);
        x.type && this.setViewType(x.type);
        x.sid && this.setMessage([]);
        this.time = [];
      }
    },

    actionSetIntentMessage(x) {
      if (this.intentMessage !== x) {
        this.setIntentMessage(x);
      }
    },
    ...
    actionRequireMessage() {
      ...
      if (localMessages) {
        this.actionAppendMessage({ messages: localMessages });
      }

      const historyQueryTimes = this.queryHistoryRecords[this.sid] || 0;
      if (historyQueryTimes === 0 && (!localMessages || localMessages.length <= 3)) {
        this.queryHistory();
      }
    },

    actionAppendMessage(data = {}) {
      const newMessages = data.messages || [];
      const isHistory = data.history;
      if (isHistory) {
        this.queryHistoryMessageId = data.next;
      ...
      while (i < newMessages.length && j < oldMessages.length) {
        ...
        const toUid = toNumber(to);
        let saveUid = fromUid === uid ? toUid : fromUid;
        if (saveUid + '' !== this.sid + '' && this.viewType === 'rosterchat') {
          return; // rosterchat, 必须有一个id是 sid
        }
        if (toUid + '' !== this.sid + '' && this.viewType === 'groupchat') {
          // 群消息 to 是 群 id。。
          return; //group，to 必须是sid
        }
      ...
      this.setMessage(allMessages);
      if (!isHistory && allMessages.length !== oldMessages.length) {
        this.scroll = this.scroll + 1;
      }
    },

    actionDeleteMessage(mid) {
      const oldMessages = this.messages || [];
      const newMessages = oldMessages.filter((meta) => {
        if (meta.id == mid) return false;
        else return true;
    ...
    actionUpdateMessageTime(time) {
      this.setMessageTime(time);
    },
