...
export default {
  ...
  mounted() {
    const app_id = mainStore.im.userManage.getAppid();
    ...
    mainStore.im.rtcManage.initRTCEngine({
      server: mainStore.im.sysManage.getServers(app_id).rtc,
      id: mainStore.im.userManage.getUid(),
      caller: this.caller,
    ...
    mainStore.im.on('onRosterRTCMessage', (message) => {
      const { ext, isHistory } = message;
      const callStatus = mainStore.im.rtcManage.getInCallStatus();
      if (!isHistory && callStatus && ext && message.from != mainStore.im.userManage.getUid()) {
  ...
  computed: {
    ...
    getCallPickupTime() {
      return collectionStore.getCallPickupTime
    },
    getCallId() {
  ...
  methods: {
    ...
    micChangeStatus() {
      if (this.isGetThrough) {
        this.mic = !this.mic;
        mainStore.im.rtcManage.muteLocalAudio(!this.mic);
      }
    },
    speakerChangeStatus() {
      if (this.isGetThrough) {
    ...
    cameraChangeStatus() {
      if (this.isGetThrough) {
        ...
        mainStore.im.rtcManage.muteLocalVideo(!this.camera);
    ...
    hangupCall(active, timeout = false, peerDrop = false) {
      ...
      mainStore.im.rtcManage.leaveRoom();
      this.doHangup = true;
      this.stopPhoneRing();
    },
    getHangUpStatus() {
    ...
    getThrough() {
      if (!this.isGetThrough) {
        ...
        collectionStore.setCallPickupTime(Date.now())
  ...
  beforeDestroy() {
    ...
    mainStore.im.rtcManage.destroy();
  }
};
</script>
